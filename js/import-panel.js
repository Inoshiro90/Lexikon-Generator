...
const WORTARTEN = [
  ...
  { key: 'Adjektiv',    label: 'Adjektive',     iconName: 'sliders',   rowParser: parseAdjektivRows, open: false },
  { key: 'Präposition', label: 'Präpositionen', iconName: 'map-pin',   rowParser: parseSimpleRows,   open: false },
  { key: 'Adverb',      label: 'Adverbien',     iconName: 'clock',     rowParser: parseSimpleRows,   open: false },
  { key: 'Partikel',    label: 'Partikel',      iconName: 'layers',    rowParser: parseSimpleRows,   open: false },
];
...
function createImportPanel(config, callbacks = {}) {
  ...
  function _clearStatus() { statusEl.className = 'import-status hidden'; statusEl.innerHTML = ''; }
  ...
  function _importRows(rows, fmt) {
    ...
    try {
      ...
      if (!parsed.length) {
        _setStatus('warn', `${icon('alert-triangle', 12)} Keine gültigen Einträge gefunden.`);
        return;
      }
      _entries = parsed;
      _updateCount(parsed.length);
      _setStatus('success',
        `${icon('check', 12)} <strong>${parsed.length}</strong> Eintr${parsed.length !== 1 ? 'äge' : 'ag'} — ${escHtml(formatLabel(fmt))}`
      );
      if (callbacks.onImport) callbacks.onImport(config.key, parsed);
    } catch (err) {
  ...
  function _runPasteImport() {
    ...
    if (!text.trim()) { _setStatus('warn', `${icon('alert-triangle', 12)} Eingabe ist leer.`); return; }
    try {
  ...
  textarea.addEventListener('keydown', e => {
    if ((e.ctrlKey || e.metaKey) && e.key === 'Enter') { e.preventDefault(); _runPasteImport(); }
  });
    ...
    if (fmt === 'spreadsheet') {
      ...
      reader.onload = e => {
        try {
          ...
          _importRows(rows, formatUsed);
        } catch (err) { _setStatus('error', `${icon('x-circle', 12)} ${escHtml(err.message)}`); }
      };
      reader.onerror = () => _setStatus('error', `${icon('x-circle', 12)} Datei nicht lesbar.`);
  ...
  return {
    ...
    clear()      { _entries = []; _updateCount(0); _clearStatus(); },
  };
}
